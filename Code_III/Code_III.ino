...
void loop() {
  ...
  // -------- OUT GATE --------
  // FIXED: Only open if parking is NOT empty (Slot < TOTAL_SLOTS) AND cooldown has passed
  ...
  // FIXED: Show message when parking is empty and someone approaches exit
  if (distOut < distanceLimit && Slot == TOTAL_SLOTS && !outGateOpen && !inGateOpen) {
    lcd.clear();
    lcd.setCursor(0, 0);
